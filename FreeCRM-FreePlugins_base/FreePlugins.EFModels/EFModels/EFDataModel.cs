...
public partial class EFDataModel : DbContext
{
    ...
    // => optionsBuilder.UseSqlite("Data Source=C:\\Working\\FreePlugins.db");
    // => optionsBuilder.UseMySQL("Server=localhost;Database=FreePlugins;User=admin;Password=admin");
    // => optionsBuilder.UseNpgsql("Host=localhost;Database=FreePlugins;Username=postgres;Password=admin");
    // => optionsBuilder.UseSqlServer("Data Source=(local);Initial Catalog=FreePlugins;Persist Security Info=True;User ID=sa;Password=saPassword;MultipleActiveResultSets=True;TrustServerCertificate=True;");
